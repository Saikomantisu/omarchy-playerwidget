...
album art sits directly in the bar, and a click opens a small panel with
pause, skip back, and skip forward.
...
deliberately different take: no text in the bar at all, just the thumbnail
— art missing or paused looks different from art actively playing, so the
state reads at a glance without reading anything.

It binds `Quickshell.Services.Mpris` directly rather than the built-in
`omarchy.media` service: Omarchy's `firstPartyServiceFor()` is only handed
out to first-party plugins and to plugins of kind `bar` (a full custom
bar), so an ordinary third-party bar-widget plugin has no route to it.
Player selection (most-recently-active, preferring one that's actively
playing) is reimplemented locally to match.
...
- The thumbnail's border switches to the accent color while playing, and
  back to a plain border when paused — the only "text" is the art itself.
